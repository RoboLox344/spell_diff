...
if (!isMainThread) {
    ...
    try {
        ...
        const mod = await HunspellModule();
...
`;
                    ...
                    suggErrors.push(line);
                    suggErrorsBuffer.push(line);
                }
            }
            ...
            if (processed % 1000 === 0 && suggErrorsBuffer.length > 0) {

                parentPort.postMessage({
                    type: "suggest_chunk",
                    workerId,
                    data: suggErrorsBuffer.join("")
                });

                suggErrorsBuffer.length = 0;
            }
        }

        if (suggErrorsBuffer.length > 0) {
            parentPort.postMessage({
                type: "suggest_chunk",
                workerId,
                data: suggErrorsBuffer.join("")
...
const CPU_COUNT = os.default.cpus().length;
...
const suggestStream = fs.createWriteStream("./stat/suggest_progress.txt", {
    flags: "a"
});
            ...
            worker.on("message", (msg) => {
                ...
                if (msg.type === "progress") {
                    ...
                    process.stdout.write(
                        `\r[${elapsed}s] ${total}/${words.length} (${pct}%)`
                    );
                }

                else if (msg.type === "suggest_chunk") {

                    suggestStream.write(msg.data);
                }
...
const percent =
    ((totalMatch / totalCount) * 100).toFixed(2);
